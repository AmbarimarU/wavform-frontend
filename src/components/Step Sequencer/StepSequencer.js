...
function StepSequencer({
  ...
  const makeGrid = (notes) => {
      ...
      for (let j = 0; j < 8; j++) {
        ...
        if (Object.keys(container).length > 1) {
          if (container[combinedIndex]) {
            noteObject = {
              ...noteObject,
              isActive: container[combinedIndex],
            };
    ...
    // we now have 8 rows each containing 8 eighth notes
    return rows;
  };
  ...
  }, [instrumentArray, flag, instrumentChanges1, instrumentChanges2, octaveChanges1, octaveChanges2]);
