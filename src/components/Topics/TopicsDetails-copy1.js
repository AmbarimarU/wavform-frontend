...
function TopicsDetails({ user }) {
  ...
  const [completionPercentage, setCompletionPercentage] = useState(
    parseInt(localStorage.getItem(`completion-${id}`)) || 0
  );
  ...
  useEffect(() => {
    ...
    const storedCompletion =
      parseInt(localStorage.getItem(`completion-${id}`)) || 0;
    const storedIndex = parseInt(localStorage.getItem(`index-${id}`)) || 0;

    setCompletionPercentage(storedCompletion);
  ...
  const handleTopicCompletion = () => {
    const totalBulletPoints = bulletPoint.length;
    const currentBulletPointIndex = currentIndex;
    const percentage = (currentBulletPointIndex / totalBulletPoints) * 100;

    setCompletionPercentage(Math.round(percentage));
    localStorage.setItem(`completion-${id}`, Math.round(percentage));
    localStorage.setItem(`index-${id}`, currentBulletPointIndex);



    console.log(`Topic completed! You achieved ${Math.round(percentage)}%`);
