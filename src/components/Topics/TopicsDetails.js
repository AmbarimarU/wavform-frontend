...
function TopicsDetails({ user }) {
  ...
  // checks if the first elements of an array is null or undefined
  const showPiano = pianoTopics.includes(singleTopic?.[0]?.name);
  const showBarAndNotesImg = barAndNoteTopic.includes(singleTopic?.[0]?.name);
  const showTempo = tempoTopic.includes(singleTopic?.[0]?.name);
